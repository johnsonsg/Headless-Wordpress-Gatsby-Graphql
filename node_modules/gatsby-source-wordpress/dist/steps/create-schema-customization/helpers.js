...
const typeSettingCache = {}; // retrieves plugin settings for the provided type
...
const getTypeSettingsByType = type => {
  ...
  const allTypeSettings = _store.default.getState().gatsbyApi.pluginOptions.type;

  const typeSettings = (0, _lodash.cloneDeep)(allTypeSettings[typeName] || {}); // the type.__all plugin option which is applied to every type setting

  const __allTypeSetting = (0, _lodash.cloneDeep)(allTypeSettings.__all || {});
  ...
  if (typeSettings) {
    const mergedSettings = (0, _lodash.merge)(__allTypeSetting, typeSettings);
