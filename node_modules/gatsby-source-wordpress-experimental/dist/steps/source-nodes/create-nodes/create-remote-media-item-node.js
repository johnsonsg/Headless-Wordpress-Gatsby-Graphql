...
const getMediaItemEditLink = node => {
  ...
  if (!databaseId) {
    ...
    if (!(parentNode === null || parentNode === void 0 ? void 0 : parentNode.databaseId)) {
      return null;
    }
...
const errorPanicker = ({
  ...
  if ((allow404ImagesInProduction || process.env.NODE_ENV !== `production`) && errorString.includes(`Response code 404`)) {
    fetchState.shouldBail = true;
    reporter.log(``);
    reporter.warn((0, _formatLogMessage.formatLogMessage)(`Error ${sharedError}${!allow404ImagesInProduction ? `\n\nThis error will fail production builds.` : ``}`));
    reporter.log(``);
    return;
  }
  ...
  const remoteFileNode = await (0, _asyncRetry.default)(async () => {
    ...
    if (hardCacheMediaFiles) {
      ...
      try {
        ...
        const node = await (0, _gatsbySourceFilesystem.createFileNodeFromBuffer)({
          buffer,
          name: title,
...
//# sourceMappingURL=create-remote-media-item-node.js.map